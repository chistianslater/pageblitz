...
function DynamicNav({ websiteData, cs, businessPhone, logoUrl }: { websiteData: WebsiteData; cs: ColorScheme; businessPhone?: string | null; logoUrl?: string | null }) {
  return (
    <nav data-section="header" style={{ backgroundColor: `${cs.background}F2`, backdropFilter: "blur(8px)", borderBottom: `1px solid ${cs.onBackground}10` }} className="sticky top-0 z-50">
        ...
        <div className="flex items-center gap-2">
          {logoUrl ? (
            ...
            <>
              ...
              <span style={{ fontFamily: LOGO_FONT, fontSize: "1.6rem", letterSpacing: "0.08em", color: cs.onBackground }}>{websiteData.businessName.toUpperCase()}</span>
            </>
          )}
        </div>
        <div className="hidden md:flex items-center gap-8">
          {["Angebot", "Über uns", "Kontakt"].map(label => (
            <a key={label} href={`#${label.toLowerCase()}`} style={{ fontFamily: BODY, fontSize: "0.9rem", letterSpacing: "0.1em", textTransform: "uppercase", color: cs.onBackground, opacity: 0.6, fontWeight: 600 }} className="hover:text-primary transition-colors">{label}</a>
          ))}
        </div>
        {businessPhone && (
          <a href={`tel:${businessPhone}`} style={{ backgroundColor: cs.primary, color: cs.onPrimary, padding: "0.6rem 1.25rem", fontFamily: HEADING, fontSize: "1rem", letterSpacing: "0.08em" }} className="hidden sm:flex items-center gap-2 hover:opacity-90 transition-opacity shadow-lg shadow-primary/20">
...
function DynamicHero({ section, cs, heroImageUrl, showActivateButton, onActivate }: { section: WebsiteSection; cs: ColorScheme; heroImageUrl: string; showActivateButton?: boolean; onActivate?: () => void; websiteData: WebsiteData }) {
  return (
    <section style={{ backgroundColor: cs.onBackground, minHeight: "100vh", position: "relative", overflow: "hidden" }} className="flex items-center pt-24">
      <div style={{ position: "absolute", inset: 0, zIndex: 0 }}>
        <img src={heroImageUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", opacity: 0.4 }} />
        <div style={{ position: "absolute", inset: 0, background: `radial-gradient(circle, transparent 20%, ${cs.onBackground} 100%)` }} />
      </div>
      
      <div className="tech-glow absolute" style={{ top: "20%", left: "50%", width: "40vw", height: "40vw", background: cs.primary, opacity: 0.15, transform: "translateX(-50%)", zIndex: 0 }} />
      
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-20 relative z-10 w-full text-center">
        <div style={{ display: "inline-flex", alignItems: "center", gap: "0.75rem", padding: "0.5rem 1.25rem", borderRadius: "9999px", backgroundColor: `${cs.background}10`, border: `1px solid ${cs.background}20`, marginBottom: "3rem" }} className="hero-animate-badge">
          <Zap className="h-4 w-4" style={{ color: cs.primary }} />
          <span style={{ fontFamily: BODY, fontSize: "0.75rem", letterSpacing: "0.2em", textTransform: "uppercase", color: cs.background, fontWeight: 700 }}>Innovation & Leistung</span>
        </div>
        ...
        <h1 style={{ 
          ...
          <span style={{ 
            color: cs.primary, 
            background: `linear-gradient(to right, ${cs.primary}, ${cs.background})`,
        ...
        {section.subheadline && (
          <p style={{ fontFamily: BODY, fontSize: "1.25rem", color: cs.background, opacity: 0.7, maxWidth: "600px", lineHeight: 1.6, fontWeight: 500, margin: "0 auto 3.5rem auto" }} className="hero-animate-sub">
            {section.subheadline}
          </p>
        )}
        ...
        <div className="flex flex-wrap gap-6 justify-center hero-animate-cta">
          {section.ctaText && (
            <a href={section.ctaLink || "#kontakt"} 
              style={{ backgroundColor: cs.primary, color: cs.onPrimary, padding: "1.25rem 4rem", fontFamily: HEADING, fontSize: "1.3rem", letterSpacing: "0.1em", textTransform: "uppercase", transition: "all 0.4s ease" }} 
          ...
          {showActivateButton && (
            <button onClick={onActivate} 
              style={{ border: `2px solid ${cs.background}33`, color: cs.background, padding: "1.25rem 4rem", fontFamily: HEADING, fontSize: "1.3rem", letterSpacing: "0.1em", textTransform: "uppercase", backgroundColor: "transparent" }} 
    ...
    <section style={{ backgroundColor: cs.background, padding: "10rem 0", position: "relative" }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8 grid lg:grid-cols-2 gap-24 items-center relative z-10">
        <div>
          ...
          <p style={{ fontFamily: BODY, fontSize: "1.15rem", lineHeight: 1.7, color: cs.onBackground, opacity: 0.8, marginBottom: "1.5rem", fontWeight: 500 }}>
            {section.subheadline}
          </p>
          <p style={{ fontFamily: BODY, fontSize: "1rem", lineHeight: 1.8, color: cs.onBackground, opacity: 0.6 }}>
            {section.content}
          </p>
        </div>
    ...
    <section data-section="gallery" style={{ backgroundColor: cs.background, padding: "6rem 0" }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex items-center gap-4 mb-12">
          <Activity className="h-6 w-6" style={{ color: cs.primary }} />
          <h2 data-reveal data-delay="100" style={{ fontFamily: HEADING, fontSize: "clamp(2.5rem, 5vw, 4rem)", fontWeight: 400, color: cs.onBackground, textTransform: "uppercase", letterSpacing: "0.02em" }}>{section.headline}</h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2 lg:gap-4">
          {items.map((item, i) => (
            <div key={i} style={{ aspectRatio: "1/1", overflow: "hidden", border: `1px solid ${cs.primary}40`, position: "relative", backgroundColor: cs.surface }} className="group">
              <img src={item.imageUrl || `https://images.unsplash.com/photo-${1534438327276 + i}?w=800&q=80&fit=crop`} alt={item.title || ""} style={{ width: "100%", height: "100%", objectFit: "cover", filter: "brightness(0.8)" }} className="group-hover:scale-105 transition-transform duration-500" />
...
function DynamicTestimonials({ section, cs }: { section: WebsiteSection; cs: ColorScheme }) {
  ...
  return (
    <section style={{ backgroundColor: cs.primary, padding: "5rem 0", position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", inset: 0, opacity: 0.05, backgroundImage: `repeating-linear-gradient(45deg, ${cs.onPrimary} 0, ${cs.onPrimary} 1px, transparent 0, transparent 50%)`, backgroundSize: "20px 20px" }} />
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        <h2 data-reveal data-delay="200" style={{ fontFamily: HEADING, fontSize: "clamp(2.5rem, 5vw, 4rem)", fontWeight: 400, color: cs.onPrimary, textTransform: "uppercase", letterSpacing: "0.02em", marginBottom: "3rem" }}>{section.headline}</h2>
        <div className="grid md:grid-cols-3 gap-6">
          {items.map((item, i) => (
            <div key={i} style={{ backgroundColor: `${cs.onPrimary}15`, padding: "2rem", backdropFilter: "blur(4px)", border: `1px solid ${cs.onPrimary}10` }}>
              <div className="flex gap-1 mb-3">{Array.from({ length: 5 }).map((_, j) => <Star key={j} className="h-4 w-4 fill-current text-yellow-300" />)}</div>
              <p style={{ fontFamily: BODY, fontSize: "0.95rem", lineHeight: 1.7, color: cs.onPrimary, opacity: 0.9, marginBottom: "1.5rem", fontWeight: 500 }}>{item.description || item.title}</p>
              <p style={{ fontFamily: HEADING, fontSize: "1rem", color: cs.onPrimary, opacity: 0.7, letterSpacing: "0.08em" }}>{item.author || item.title}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <section style={{ backgroundColor: cs.surface, padding: "5rem 0" }}>
      <div className="max-w-4xl mx-auto px-6 lg:px-8">
        <h2 data-reveal data-delay="300" style={{ fontFamily: HEADING, fontSize: "clamp(2.5rem, 5vw, 4rem)", fontWeight: 400, color: cs.onSurface, textTransform: "uppercase", letterSpacing: "0.02em", marginBottom: "3rem" }}>{section.headline}</h2>
        <div className="space-y-2">
          {items.map((item, i) => (
            <div key={i} style={{ borderLeft: `3px solid ${open === i ? cs.primary : `${cs.onSurface}20`}`, paddingLeft: "1.5rem", transition: "border-color 0.2s" }}>
              <button onClick={() => setOpen(open === i ? null : i)} className="w-full text-left py-4 flex items-center justify-between" style={{ fontFamily: HEADING, fontSize: "1.1rem", color: cs.onSurface, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                {item.question || item.title}
                {open === i ? <ChevronUp className="h-5 w-5 flex-shrink-0" style={{ color: cs.primary }} /> : <ChevronDown className="h-5 w-5 flex-shrink-0" style={{ color: cs.onSurface, opacity: 0.4 }} />}
              </button>
              {open === i && <p style={{ fontFamily: BODY, fontSize: "0.9rem", lineHeight: 1.7, color: cs.onSurface, opacity: 0.6, paddingBottom: "1rem", fontWeight: 500 }}>{item.answer || item.description}</p>}
            </div>
          ))}
        </div>
      </div>
...
function DynamicMenu({ section, cs }: { section: WebsiteSection; cs: ColorScheme }) {
  ...
  return (
    <section style={{ backgroundColor: cs.background, padding: "6rem 0", position: "relative", overflow: "hidden" }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "4rem" }}>
          ...
          <h2 style={{ fontFamily: HEADING, fontSize: "clamp(2.5rem, 6vw, 4.5rem)", fontWeight: 400, color: cs.onBackground, textTransform: "uppercase", letterSpacing: "0.02em" }}>{section.headline}</h2>
        </div>
                  ...
                  {items.filter(item => item.category === cat).map((item, i) => (
                    <div key={i} className="group flex justify-between items-baseline gap-4">
                      <div className="flex-1">
                        <h4 style={{ fontFamily: HEADING, fontSize: "1.2rem", fontWeight: 400, color: cs.onBackground, textTransform: "uppercase", letterSpacing: "0.05em" }}>{item.title}</h4>
                        {item.description && (
                          <p style={{ fontFamily: BODY, fontSize: "0.9rem", color: cs.onBackground, opacity: 0.5, marginTop: "0.25rem" }}>{item.description}</p>
                        )}
                      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        {categories.length > 0 ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories.map((cat, idx) => (
              <div key={idx} style={{ backgroundColor: cs.background, padding: "2.5rem", borderTop: `4px solid ${cs.primary}`, borderLeft: `1px solid ${cs.onBackground}05`, borderRight: `1px solid ${cs.onBackground}05`, borderBottom: `1px solid ${cs.onBackground}05` }}>
                ...
                <div className="space-y-4">
                  {items.filter(item => item.category === cat).map((item, i) => (
                    <div key={i} className="flex justify-between items-center py-2 border-b last:border-0" style={{ borderColor: `${cs.onBackground}05` }}>
                      <span style={{ fontFamily: BODY, fontSize: "1rem", color: cs.onBackground, fontWeight: 600, textTransform: "uppercase" }}>{item.title}</span>
          ...
          <div style={{ backgroundColor: cs.background, padding: "3rem", borderTop: `4px solid ${cs.primary}`, borderLeft: `1px solid ${cs.onBackground}05`, borderRight: `1px solid ${cs.onBackground}05`, borderBottom: `1px solid ${cs.onBackground}05`, maxWidth: "800px", margin: "0 auto" }}>
            <div className="space-y-6">
              {items.map((item, i) => (
                <div key={i} className="flex justify-between items-center py-4 border-b last:border-0" style={{ borderColor: `${cs.onBackground}05` }}>
                  <span style={{ fontFamily: BODY, fontSize: "1.15rem", color: cs.onBackground, fontWeight: 600, textTransform: "uppercase" }}>{item.title}</span>
    ...
    <section id="kontakt" style={{ backgroundColor: cs.background, padding: "5rem 0" }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8 grid lg:grid-cols-2 gap-12">
        ...
        <div style={{ backgroundColor: cs.surface, padding: "2.5rem", borderTop: `4px solid ${cs.primary}`, borderLeft: `1px solid ${cs.onSurface}05`, borderRight: `1px solid ${cs.onSurface}05`, borderBottom: `1px solid ${cs.onSurface}05` }}>
          ...
          >
            <input type="text" placeholder="NAME" style={{ width: "100%", backgroundColor: `${cs.onSurface}05`, border: `1px solid ${cs.onSurface}10`, padding: "1rem", color: cs.onSurface, fontFamily: HEADING, fontSize: "0.9rem", letterSpacing: "0.05em", outline: "none" }} />
            <input type="email" placeholder="E-MAIL" style={{ width: "100%", backgroundColor: `${cs.onSurface}05`, border: `1px solid ${cs.onSurface}10`, padding: "1rem", color: cs.onSurface, fontFamily: HEADING, fontSize: "0.9rem", letterSpacing: "0.05em", outline: "none" }} />
            <textarea placeholder="WIE KÖNNEN WIR DIR HELFEN?" rows={4} style={{ width: "100%", backgroundColor: `${cs.onSurface}05`, border: `1px solid ${cs.onSurface}10`, padding: "1rem", color: cs.onSurface, fontFamily: HEADING, fontSize: "0.9rem", letterSpacing: "0.05em", outline: "none", resize: "none" }} />
            <button type="submit" style={{ backgroundColor: cs.primary, color: cs.onPrimary, padding: "1.1rem", fontFamily: HEADING, fontSize: "1rem", fontWeight: 700, letterSpacing: "0.1em", border: "none", cursor: "pointer", textTransform: "uppercase" }} className="hover:opacity-90 transition-opacity">
          ...
          <div className="space-y-2">
            {(hours.length > 0 ? hours : ["Mo – Fr: 06:00 – 22:00 Uhr", "Sa – So: 08:00 – 20:00 Uhr"]).map((h, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <p style={{ fontFamily: BODY, fontSize: "0.9rem", color: cs.onSurface, opacity: 0.7, fontWeight: 500 }}>{h}</p>
              </div>
            ))}
          </div>
        </div>
...
function DynamicCTA({ section, cs, showActivateButton, onActivate }: { section: WebsiteSection; cs: ColorScheme; showActivateButton?: boolean; onActivate?: () => void }) {
  return (
    <section style={{ backgroundColor: cs.surface, padding: "5rem 0", position: "relative", overflow: "hidden", borderTop: `1px solid ${cs.onSurface}05` }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-8">
        <div>
          <h2 data-reveal data-delay="300" style={{ fontFamily: HEADING, fontSize: "clamp(2.5rem, 5vw, 4.5rem)", fontWeight: 400, color: cs.onSurface, textTransform: "uppercase", letterSpacing: "0.02em", lineHeight: 0.95 }}>{section.headline}</h2>
          {section.content && <p style={{ fontFamily: BODY, fontSize: "1rem", color: cs.onSurface, opacity: 0.7, marginTop: "1rem", fontWeight: 500 }}>{section.content}</p>}
        </div>
        <div className="flex flex-wrap gap-4 flex-shrink-0">
          {section.ctaText && <a href={section.ctaLink || "#kontakt"} style={{ backgroundColor: cs.primary, color: cs.onPrimary, padding: "1rem 3rem", fontFamily: HEADING, fontSize: "1.2rem", letterSpacing: "0.1em", textTransform: "uppercase" }} className="hover:opacity-90 transition-opacity shadow-xl shadow-primary/20">{section.ctaText}</a>}
  ...
  return (
    <footer data-section="footer" style={{ backgroundColor: cs.onBackground, padding: "2.5rem 0", borderTop: `3px solid ${cs.primary}` }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <Zap className="h-4 w-4" style={{ color: cs.primary }} />
          <span style={{ fontFamily: HEADING, fontSize: "1.3rem", color: cs.background, letterSpacing: "0.08em" }}>{websiteData.businessName.toUpperCase()}</span>
        </div>
        <p style={{ fontFamily: BODY, fontSize: "0.8rem", color: cs.background, opacity: 0.5, fontWeight: 500 }}>{websiteData.footer?.text}</p>
        <p style={{ fontFamily: BODY, fontSize: "0.75rem", color: cs.background, opacity: 0.4 }}>Erstellt mit <span style={{ color: cs.primary }}>Pageblitz</span></p>
      </div>
    </footer>
  );
}
