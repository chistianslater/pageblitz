...
export default function LuxuryLayout({ websiteData, cs, heroImageUrl, showActivateButton, onActivate, businessPhone, businessAddress, businessEmail, openingHours = [] }: Props) {
  ...
  return (
    <div style={{ fontFamily: BODY, backgroundColor: darkCs.background, color: darkCs.text }}>
      ...
      {websiteData.sections.map((section, i) => (
        <div key={i}>
          {section.type === "hero" && <LuxuryHero section={section} cs={darkCs} heroImageUrl={heroImageUrl} showActivateButton={showActivateButton} onActivate={onActivate} />}
          ...
          {section.type === "contact" && <LuxuryContact section={section} cs={darkCs} phone={businessPhone} address={businessAddress} email={businessEmail} hours={openingHours} />}
          {section.type === "cta" && <LuxuryCTA section={section} cs={darkCs} showActivateButton={showActivateButton} onActivate={onActivate} />}
        </div>
      ))}
      <LuxuryFooter websiteData={websiteData} cs={darkCs} />
    </div>
  );
}
...
function LuxuryHero({ section, cs, heroImageUrl, showActivateButton, onActivate }: { section: WebsiteSection; cs: ColorScheme; heroImageUrl: string; showActivateButton?: boolean; onActivate?: () => void }) {
  return (
...
function LuxuryFooter({ websiteData, cs }: { websiteData: WebsiteData; cs: ColorScheme }) {
  return (
