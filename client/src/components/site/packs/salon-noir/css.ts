...
.pb-salon-noir{--pb-shell-outer:100%;background:var(--pb-canvas);color:var(--pb-ink);font-family:var(--pb-font-body);font-weight:300;line-height:1.6;overflow-x:clip;position:relative}
...
.pb-salon-noir{--pb-shell-pad:40px}
...
.pb-sn-gallery{display:grid;grid-template-columns:1.15fr .85fr;gap:clamp(16px,3vw,42px);align-items:start}
.pb-sn-gallery figure{margin:0}.pb-sn-gallery figure:nth-child(2){margin-top:clamp(46px,8vw,120px)}.pb-sn-gallery img{width:100%;height:clamp(340px,48vw,680px);object-fit:cover;border:1px solid var(--pb-line);display:block;filter:saturate(.72) contrast(1.04)}
