...
.pb-pa-gallery{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));gap:clamp(14px,2.5vw,36px);align-items:start}
.pb-pa-gallery img{width:100%;height:clamp(320px,44vw,620px);object-fit:cover;border-radius:200px 200px var(--pb-radius-card) var(--pb-radius-card);display:block;filter:saturate(.78) sepia(.05)}.pb-pa-gallery img:nth-child(1){grid-column:1/span 7}.pb-pa-gallery img:nth-child(2){grid-column:8/span 5;margin-top:clamp(48px,9vw,130px)}.pb-pa-gallery img:nth-child(3){grid-column:3/span 8;height:clamp(300px,38vw,540px)}
