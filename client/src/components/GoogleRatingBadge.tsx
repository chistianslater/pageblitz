/**
 * GoogleRatingBadge – Displays real Google rating data prominently.
 * Used in hero sections across all 12 layout components.
 *
 * Shows: Google logo mark + star rating + review count
 * Adapts: light/dark variants based on background color
 */
import { Star } from "lucide-react";

interface Props {
  rating?: number;
  reviewCount?: number;
  /** "light" = badge on dark bg (white text), "dark" = badge on light bg (dark text) */
  variant?: "light" | "dark";
  /** Optional override for accent/star color */
  starColor?: string;
  className?: string;
}

export default function GoogleRatingBadge({ rating, reviewCount, variant = "dark", starColor = "#f59e0b", className = "" }: Props) {
  if (!rating && !reviewCount) return null;
  const textColor = variant === "light" ? "rgba(255,255,255,0.95)" : "#1a1a1a";
  const subColor = variant === "light" ? "rgba(255,255,255,0.65)" : "#666";
  const bgColor = variant === "light" ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.05)";
  const borderColor = variant === "light" ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.1)";

  const stars = Math.round(rating ?? 0);

  return (
    <div
      className={`inline-flex items-center gap-3 ${className}`}
      style={{
        backgroundColor: bgColor,
        border: `1px solid ${borderColor}`,
        borderRadius: "0.5rem",
        padding: "0.6rem 1rem",
        backdropFilter: "blur(8px)",
      }}
    >
      {/* Google "G" mark */}
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ flexShrink: 0 }}>
        <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
        <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
        <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
        <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
      </svg>

      {/* Stars */}
      <div className="flex items-center gap-0.5">
        {Array.from({ length: 5 }).map((_, i) => (
          <Star
            key={i}
            className="h-3.5 w-3.5"
            style={{
              color: i < stars ? starColor : (variant === "light" ? "rgba(255,255,255,0.3)" : "#ddd"),
              fill: i < stars ? starColor : "transparent",
            }}
          />
        ))}
      </div>

      {/* Rating + count */}
      <div className="flex items-baseline gap-1.5">
        <span style={{ fontWeight: 700, fontSize: "0.95rem", color: textColor, lineHeight: 1 }}>
          {(rating ?? 0).toFixed(1)}
        </span>
        <span style={{ fontSize: "0.75rem", color: subColor, lineHeight: 1 }}>
          ({(reviewCount ?? 0).toLocaleString("de-DE")} Bewertungen)
        </span>
      </div>
    </div>
  );
}

/**
 * Compact inline version for use inside text blocks
 */
export function GoogleRatingInline({ rating, reviewCount, color = "#f59e0b" }: { rating: number; reviewCount: number; color?: string }) {
  return (
    <span className="inline-flex items-center gap-1.5" style={{ fontSize: "0.85rem" }}>
      <Star className="h-3.5 w-3.5 fill-current" style={{ color }} />
      <strong style={{ color }}>{rating.toFixed(1)}</strong>
      <span style={{ color: "inherit", opacity: 0.7 }}>({reviewCount.toLocaleString("de-DE")} Google-Bewertungen)</span>
    </span>
  );
}
