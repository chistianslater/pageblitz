...
export default function CustomerLoginPage() {
  ...
  return (
    <LoginShell
      ...
      footer={
        <>
          ...
          <a
            href="/start"
            className="font-medium text-lp-accent-ink underline decoration-lp-accent-ink/35 underline-offset-4"
          >
    ...
    >
      {(error || urlError) && (
        <div className="mb-6 flex items-start gap-2 rounded-[10px] border border-[rgba(164,68,31,0.35)] bg-[rgba(164,68,31,0.07)] p-3">
      ...
      {magicLinkSent ? (
        <div className="py-3 text-center">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-lp-accent">
            <Mail className="h-7 w-7 text-lp-accent-ink" />
          </div>
        ...
        <>
          <button
            ...
            disabled={loading}
            className="mb-4 flex w-full items-center justify-center gap-3 rounded-full border border-lp-line bg-lp-canvas px-4 py-3.5 font-medium text-lp-ink transition-colors hover:border-lp-ink disabled:cursor-not-allowed disabled:opacity-50"
          >
            {loading ? (
          ...
          <form onSubmit={handleMagicLink} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  required
                  className="h-12 border-lp-line bg-lp-canvas pl-10 text-lp-ink placeholder:text-lp-muted focus-visible:border-lp-accent-ink"
                />
              </div>
            </div>
            ...
            <Button
              ...
              className="h-12 w-full rounded-full bg-lp-accent py-3 font-medium text-lp-accent-ink hover:bg-[#c2e024]"
            >
              {loading ? (
                <span className="h-5 w-5 animate-spin rounded-full border-2 border-lp-accent-ink/30 border-t-lp-accent-ink motion-reduce:animate-none" />
              ) : (
                <>
                  Login-Link senden
