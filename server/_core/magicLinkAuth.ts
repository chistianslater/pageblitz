...
export function registerMagicLinkAuthRoutes(app: Express) {
  ...
  app.post("/api/auth/magic-link", async (req: Request, res: Response) => {
    const { email, redirect } = req.body as {
      email?: string;
      redirect?: string;
    };
    ...
    try {
      ...
      const recent = await db.countRecentMagicLinksByEmail(
      ...
      const token = await db.createMagicLinkToken(
        email.toLowerCase().trim(),
        redirectUrl
      );
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        try {
          await linkOrphanSubscriptionsToUser(user.id, email);
        } catch (linkErr) {
          console.error(
            "[MagicLink] Fehler beim Binden verwaister Abos:",
            linkErr
          );
        }
