  ...
  outreach: router({
    ...
    send: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const { wrapPageblitzEmail, emailFooter } = await import(
          "./_core/emailDesign"
        );
        const htmlBody = wrapPageblitzEmail({
          eyebrow: "Persönlich von Christian",
          content: input.body
            .split("\n\n")
            .map(
              p =>
                `<p style="margin:0 0 16px;color:#3f3a34;font-size:15px;line-height:1.7;">${p.replace(/\n/g, "<br>")}</p>`
            )
            .join(""),
          footer: emailFooter({
            note: 'Gesendet von Christian Slater, Gr&uuml;nder von Pageblitz<br><a href="mailto:christian@pageblitz.de?subject=Abmelden" style="color:#6b645b;">Abmelden</a>',
          }),
        });
